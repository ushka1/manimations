...
class StirlingICompairsion(mn.ThreeDScene):
    run_animations = True
    ...
    def first_scene(self):
        ...
        C = np.vectorize(get_stirling_first_kind)(N_grid, K_grid)
        C = C / scaler

        axes = mn.ThreeDAxes(
            x_range=[0, n_max + 1, 1],
            y_range=[0, k_max + 1, 1],
            z_range=[0, math.ceil(C.max()), 1],
        )

        labels = axes.get_axis_labels(
            x_label=mn.MathTex("k").scale(3),
            y_label=mn.MathTex("n").scale(3),
            z_label=mn.MathTex("C(n, k)").scale(3)
        )
        ...
        for k in K:
            ...
            ticks.add(tick)
        for s in range(math.ceil(C.max())):
